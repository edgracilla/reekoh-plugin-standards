...
class ReekohYml {
    ...
    return Promise.props({
      ...
      return Promise.each(Object.keys(rule), ruleKey => {
        ...
        return Promise.each(Object.keys(root.metadata), metaKey => {
          switch (metaKey) {
            case 'icon':
              this.anchorFileCheck(errBag, metaKey, root.metadata[metaKey], 'assets')
              break
            ...
            case 'release':
              return Promise.each(Object.keys(root.metadata.release), releaseKey => {
                if (~['documentation', 'notes'].indexOf(releaseKey)) {
                  ...
                  this.anchorFileCheck(errBag, releaseKey, root.metadata[metaKey][releaseKey], folder)
                }
              })
          }
        })
  ...
  anchorFileCheck (errBag, key, value, folder) {
    ...
    if (folder) {
      ...
      if (!regexCorrectFolder.test(value)) {
        errBag.push(`'${key}' folder value must be '${folder}'`)
      }
      if (key === 'icon') {
        if (!regexCorrectPath.test(value)) {
          errBag.push(`'${key}' value should be './${folder}/${this.pluginName}.${ext}'`)
        }
      }
    }
