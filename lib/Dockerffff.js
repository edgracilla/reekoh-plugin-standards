...
class Dockerfile {
  ...
  check () {
    let errBag = []
    let inRoot = fs.existsSync(`${this.root}/Dockerfile`)
    let inRule = fs.existsSync(`${__dirname}/rules/Dockerfile`)

    if (!inRule) {
      return Promise.reject(new Error('Dockerfile rule not set!'))
    }

    if (!inRoot) {
      errBag.push(`Dockerfile file not in plugin root dir`)
      return Promise.resolve(errBag)
    }

    return Promise.props({
      rootContent: this.readToObject(`${this.root}/Dockerfile`),
      ruleContent: this.readToObject(`${__dirname}/rules/Dockerfile`)
    }).then(result => {
        ...
        switch (ruleKey) {
          case 'FROM':
            if (!root.FROM) {
              ...
              errBag.push(`'FROM' value should be '${rule.FROM}'`)
            }
            break
          ...
          case 'MAINTAINER':
            if (!root.MAINTAINER) {
              ...
              errBag.push(`'MAINTAINER' value should be '${rule.MAINTAINER}'`)
            }
            break
          ...
          case 'CMD': // *!
            if (!root.CMD) {
              ...
              errBag.push(`'CMD' value should be '${rule.CMD}'`)
            }
            break
          ...
          case 'COPY': // *!
            if (!root.COPY) {
              ...
              errBag.push(`'COPY' value should be '${rightValue}'`)
            }
            break
          ...
          case 'WORKDIR': // *!
            if (!root.WORKDIR) {
              ...
              errBag.push(`'WORKDIR' value should be '${rightValue}'`)
            }
            break
            ...
            if (!root.RUN) {
              ...
              if (typeof root.RUN === 'string') {
                if (rule.RUN !== root.RUN) {
                  errBag.push(`'RUN' value should be '${rule.RUN}'`)
                }
                ...
                if (~root.RUN.indexOf('npm install --production') || ~root.RUN.indexOf('npm i --production')) {
                  console.log(`WARNING: 'npm install --production' found in 'Dockerfile' verify that this plugin has C bindings.`)
                }
                if (!~root.RUN.indexOf(rule.RUN)) {
                  errBag.push(`'${rule.RUN}' value should be in you 'RUN' commands `)
                }
